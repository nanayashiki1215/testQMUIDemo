...
- (void)viewDidLoad
{
    ...
    dispatch_queue_t get_DeviceUpgradeStatus = dispatch_queue_create("getDeviceUpgradeStatus", nil);
    dispatch_async(get_DeviceUpgradeStatus, ^{
        NSString* errMsg;
        DeviceInfo* bindDeviceInfo = [[DeviceInfo alloc] init];
        RestApiService* restApiService = [RestApiService shareMyInstance];
        //[restApiService getBindDeviceInfo:m_strDevSelected Info_out:bindDeviceInfo Msg:&errMsg];
        [restApiService getBindDeviceChannelInfo:m_strDevSelected Chnl:[NSString stringWithFormat:@"%ld", (long)m_devChnSelected] Info_out:bindDeviceInfo Msg:&errMsg];
        [restApiService getDeviceVersion:m_strDevSelected Info_out:bindDeviceInfo Msg:&errMsg];
        [restApiService getStorageStrategy:m_strDevSelected Chnl:m_devChnSelected Info_out:bindDeviceInfo Msg:&errMsg];
        if ([errMsg isEqualToString:[MSG_SUCCESS mutableCopy]]) {
            dispatch_sync(dispatch_get_main_queue(), ^{
                if (!(bindDeviceInfo->canBeUpgrade)) {
                    _m_upgradeBtn.hidden = YES;
                }
            });
            return;
        }
