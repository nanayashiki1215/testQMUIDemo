...
/// 以下是 QMUI 提供的用于代替 DefLog() 的打 log 的方法，可根据 logName、logLevel 两个维度来控制某些 log 是否要被打印，以便在调试时去掉不关注的 log。
...
//#ifdef DEBUG
//
//// iOS 11 之前用真正的方法替换去实现拦截 DefLog 的功能，iOS 11 之后这种方法失效了，所以只能用宏定义的方式覆盖 DefLog。这也就意味着在 iOS 11 下一些如果某些代码编译时机比 QMUI 早，则这些代码里的 DefLog 是无法被替换为 QMUILog 的
//extern void _NSSetLogCStringFunction(void (*)(const char *string, unsigned length, BOOL withSyslogBanner));
//static void PrintDefLogMessage(const char *string, unsigned length, BOOL withSyslogBanner) {
//    QMUILog(@"DefLog", @"%s", string);
//}
//
//static void HackDefLog(void) __attribute__((constructor));
//static void HackDefLog(void) {
//    _NSSetLogCStringFunction(PrintDefLogMessage);
//}
//
//#define DefLog(...) QMUILog(@"DefLog", __VA_ARGS__)// iOS 11 以后真正生效的是这一句
//#endif
