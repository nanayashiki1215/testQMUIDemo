...
- (void)returnLocationInfo:(NSString*)callbackId andKeepCallback:(BOOL)keepCallback
{
    ...
    if (lData && !lData.locationInfo) {
      ...
      [geocoder reverseGeocodeLocation:lInfo completionHandler:^(NSArray *placemark,NSError *error)
       {
         ...
         NSString *MyAddress = @"未知位置";
         if(placemark.count > 0){
           CLPlacemark *mark = [placemark objectAtIndex:0];
           if([mark.addressDictionary objectForKey:@"FormattedAddressLines"] != NULL){
