...
- (UITableViewCell *)qmui_tableView:(UITableView *)tableView cellWithIdentifier:(NSString *)identifier {
    if ([identifier isEqualToString:kCellIdentifier]) {
        BGQMEventDynamicTableViewCell *cell = (BGQMEventDynamicTableViewCell *)[tableView dequeueReusableCellWithIdentifier:identifier];
        if (!cell) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    BGQMEventDynamicTableViewCell *cell = (BGQMEventDynamicTableViewCell *)[self qmui_tableView:tableView cellWithIdentifier:kCellIdentifier];
    NSString *keyName = self.dataSource.allKeys[indexPath.row];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    id<NSCopying> cachedKey = [self cachedKeyAtIndexPath:indexPath];
    NSString *keyName = self.dataSource.allKeys[indexPath.row];
    return [tableView qmui_heightForCellWithIdentifier:kCellIdentifier cacheByKey:cachedKey configuration:^(BGQMEventDynamicTableViewCell *cell) {
        [cell updateCellAppearanceWithIndexPath:indexPath];
        [cell renderWithNameText:[NSString stringWithFormat:@"%@ - %@", @(indexPath.row), keyName] contentText:[self.dataSource objectForKey:keyName]];
    }];
}
